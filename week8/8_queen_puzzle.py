...
class BuzzleApp(kivy.app.App):
    ...
    def update_board_UI(self, *args):
        ...
        # if str(numpy.round(max_fitness, 2)) == 1.1:
        if numpy.round(max_fitness, 2) == 1.1:
            # self.num_attacks_Label.text = "Max Fitness = " + str(numpy.round(max_fitness, 4)) + "\n" + "Attacks = 0"
            self.num_attacks_Label.text = "Max Fitness = " + "inf" + "\n" + "Attacks = 0"
        else:
            self.num_attacks_Label.text = "Max Fitness = " + str(numpy.round(max_fitness, 4)) + "\n" + "Attacks = " + f"{1/self.ga_instance.best_solution()[1]}"
    ...
    def build(self):
        ...
        initial_button = kivy.uix.button.Button(text="Initial Population", font_size=30, size_hint_x=2)
        initial_button.bind(on_press=self.initialize_population)

        ga_solution_button = kivy.uix.button.Button(text="Show Best Solution", font_size=30, size_hint_x=2)
        ga_solution_button.bind(on_press=self.update_board_UI)

        start_ga_button = kivy.uix.button.Button(text="Start GA", font_size=30, size_hint_x=2)
...
def fitness(ga_instance, solution_vector, solution_idx):
